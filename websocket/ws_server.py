...
def get_datalength(msg):
    '''此函数在建立websocket会话，接受用户发送的数据后调用，通过解包接收到的bytes信息计算出用户发送过来的数据总长度及数据帧头部的大小。
    websocket帧在封装不同长度的内容时头部大小是不一样的，需要此函数处理后计算出所有数据是否接收完毕。'''
...
def parser_data(msg):
    '''浏览器在建立websocket会话后发送过来的bytes数据是有掩码加密的，此函数在建立websocket会话接受完用户发送的所有数据后调用，
    提取出用户发送过来的实际内容'''
    global g_code_length
    g_code_length = msg[1] & 127
    if g_code_length == 126:
...
class WebSocketServer(object):
    ...
    def begin(self):
        ...
        port = 8080
        if PRINT_FLAG:
            print("WebServer is listening %s,%d" % ('127.0.0.1', 8005))
        self.socket.bind(('127.0.0.1', 8005))
